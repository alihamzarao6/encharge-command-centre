...
- [ ] 2.6.5 Screenshots at the three widths committed under `docs/assets/stage-2/`; `docs/RUNBOOK.md` §1 system map updated with the deployed URL and owner. *(Screenshots generated by the browser suite against the scripted backend — login, empty, conversation, cap-reached × 375 / 768 / 1280. RUNBOOK §1a has the deploy procedure (Vercel, decided on review 25 Aug); **the URL is filled in on deploy**, blocked on Supabase CLI credentials — see MEMORY 25 Aug.)*
