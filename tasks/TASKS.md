...
- [ ] **M4 data layer (brief "Milestone 4, Part 1 — GoHighLevel read and sync layer", 12 Sep) — BUILT, committed and pushed 13 Sep (`924489b`; test fixes `85a368f`, `b9e35e1`, `096b396`; `8da674d` restores a file swept into `b9e35e1`), **CI run 34739663881 on `096b396` fully green — the first execution of the part-1 stack suites with zero skipped**.** Migration `20260912010000_ghl_sync.sql` (six `ghl_*` tables, RLS forced, three service_role-only functions), `src/lib/crm/ghl/` (GET-only client, sync, store), `npm run crm -- sync|read|runs`, fixtures, 90 unit + 7 integration + 10 security tests. Part A decisions and the live-read findings: `docs/MEMORY.md` 12 Sep. *The brief's part numbering differs from the five parts below — reviewer's call.* The first CI run of the stack suites failed 4 of 7 (a stray broken definition in the changed fixture; test 5 leaking its run slot) — fixed in `85a368f`, MEMORY.md 13 Sep.
- [ ] **M4 part 1 — The screen.** The app rebuilt as a working screen rather than a chat window: the client's numbers and lists up front, the assistant as a panel beside them
  - [ ] **Brief "Milestone 4, Part 2 — Layout shell and overview screen" (13 Sep) — BUILT, awaiting review, not committed.** The overview is the landing route (`/`), the Assistant at `/assistant` (full page, unchanged) and as a docked panel from every other screen, both over one thread store (`web/src/lib/thread.ts`); nav shows only what exists (the two placeholders removed); headline numbers from the part-1 mirror (open leads, new this week, per stage by id, five most recent) with freshness, running / failed / partial / capped states and a Refresh that POSTs the new `crm` Edge Function (`src/lib/crm/ghl/page.ts`, `src/functions/crm/`). Part A decisions, states covered and the numbers: `docs/MEMORY.md` 13 Sep. Screenshots `docs/assets/milestone-4/`.
- [ ] **M4 part 2 — Leads, pipeline and stages live from GoHighLevel.** The Finance Pipeline, read by stage ID, scoped to the pipeline, the GHL token server-side only
  - [ ] **Brief "Milestone 4, Part 3 — Pipeline board and leads list" (14 Sep) — BUILT, awaiting review, not committed.** `/leads` (board primary from 768, list primary on a phone, the choice remembered; `/leads/board`, `/leads/list`, `/leads/<opportunity id>`): stages as columns in pipeline order with counts and zeros shown, cards newest first with stable ids and a drop target per column (nothing drags — part 4); the list sortable and searchable, stacked at 375; search and stage filter shared across the views; click-through to an honest lead placeholder; the refresh cooldown (60 s after the last run ended, server-side 429 + the button counting down; `src/lib/crm/cooldown.ts`); the overview's freshness, polling and refresh moved into `useMirror.ts` / `SyncStatus.tsx` and shared. Part A decisions, states covered and the numbers: `docs/MEMORY.md` 14 Sep. Screenshots `docs/assets/milestone-4/leads-*`.
