...
| **D26** | ~~**Six delivery stages replace the five phases**, everywhere: 1 GHL + Meta · 2 Foundations + AI trained on voice · 3 Memory + dashboard · 4 Website reading and storage · 5 Content, carousels, ad copy · 6 Monitoring, testing, docs, handover~~ **SUPERSEDED 1 Sep 2026 by D77** — eight milestones. Stages 1–3 became Milestones 1–3 unchanged; "Stage 4" website reading now sits inside Milestone 5 | Stage 1 is complete, signed off and paid. Phase numbering in older entries below refers to the superseded five-phase plan — read it as history, not as the current map. *Stage numbering in entries dated 22 Aug–30 Aug refers to this six-stage map — likewise history* | 22 Aug → 1 Sep |
| **D27** | ~~**Commercials: 1320 total. 198 on sign-off of each of stages 1–4, 528 at the end**~~ **SUPERSEDED 1 Sep 2026 by D77** — 2,000 USD total across eight milestones; 600 received for milestones 1–3, 1,400 remaining across 4–8 | Stage 1's 198 is **paid**. Supersedes the "5 phases, $100 upfront" line in the 07 Aug communication log | 22 Aug → 1 Sep |
...
| **D30** | **The real lead path is: Facebook ad video → FUNDD landing page → form → Discovery Session calendar booking** *(the path stands)*. ~~The funnel stays on its `sites.leadconnectorhq.com` address~~ **domain clause SUPERSEDED 7 Sep 2026 by D80: the funnel is live on `ross.fundd.com.au`** | Earlier assumptions about the entry path were wrong. The FUNDD funnel is the live landing page and ~~**stays on its `sites.leadconnectorhq.com` address** — the client will not point a custom domain at it because `fundd.com.au` belongs to his aggregator group. Do not propose a domain swap~~ *(superseded — the client obtained a subdomain from that group, D80)*. Closes R23 | 22 Aug → 7 Sep |
...
| **D77** | **The delivery plan is EIGHT MILESTONES at 2,000 USD total, agreed with the client on 1 Sep 2026 and confirmed in a milestones document he now holds. Supersedes D26 and D27.** In the client's own wording: **1** CRM and ad tracking set up (200) · **2** Foundations and the client's writing voice (200) · **3** Memory and the app itself (200) · **4** Your data on screen (350) · **5** Files, documents and research (250) · **6** Talking to it, and your day (350) · **7** Advertising and social (300) · **8** Content and handover (150). **Milestones 1, 2 and 3 are complete, signed off and paid — 600 received; 1,400 remains across milestones 4–8.** Website reading is **no longer a stage of its own**: it sits inside Milestone 5 with files and documents | Stages 1–3 map onto milestones 1–3 one to one (GHL + Meta → 1, foundations + voice → 2, memory + dashboard → 3), so nothing already built moves and every acceptance record for them stands. What changes is everything after: the old Stage 4 (website reading) is folded into Milestone 5, the old Stage 5–6 content and handover items spread across Milestones 7–8, and Milestone 4 (D78) is work the six-stage map never described. *Not reconciled in this repo:* the 22 Aug record shows **198** received for Stage 1, while the milestones document prices Milestone 1 at **200** and records **600** received across 1–3 — both left as written; the milestones document is the current agreement. Payment dates for milestones 2 and 3 are not recorded here — open | 1 Sep (recorded 12 Sep) |
| **D78** | **Milestone 4 — "Your data on screen" (350) — is five things:** (1) the app rebuilt as a working screen rather than a chat window — the client's numbers and lists up front, the assistant as a panel beside them; (2) leads, pipeline and stages showing live from GoHighLevel; (3) click into any lead and see the detail; (4) the client can move a lead from one stage to another himself; (5) works the same on a phone as on a laptop | The client's own wording from the milestones document; the old docs did not describe it at all. It is the first milestone in which the *running system* writes to GoHighLevel (a stage move), so CLAUDE.md rules 8–9 (timeouts; idempotent, keyed on the GHL opportunity ID, never blindly retried), R22/R25 (nothing account-wide — scope every read and write to the Finance Pipeline), CLIENT-CONTEXT §3 (match on stage ID, never name) and D81 (the drop is the confirmation) bind from part 1. Acceptance criteria: `PHASE-ACCEPTANCE.md` Milestone 4. Task detail arrives with each part's own brief | 1 Sep (recorded 12 Sep) |
| **D79** | **Milestone order changed at the client's request on 5 Sep 2026: the dashboard (Milestone 4) first, then what were previously the files and voice milestones (now 5 and 6), with advertising and social moved later (now 7).** The D77 table is the current order | He wants his data on screen before anything further is built on top of it. The exact pre-5 Sep order is not recorded in this repo beyond "advertising and social came earlier" — open, and of historical interest only; the milestones document as it now stands is the agreement | 5 Sep (recorded 12 Sep) |
| **D80** | **The FUNDD funnel is live on `ross.fundd.com.au` (as of 7 Sep 2026), a subdomain the client obtained from his aggregator group, and email now sends from `send.fundd.com.au`. The old `info.enchargecapital.com` address serves a different funnel entirely. Supersedes the domain clause of D30; the lead path itself (Facebook ad video → landing page → form → Discovery Session booking) stands** | D30 said no custom domain would be pointed at the funnel because `fundd.com.au` belongs to the aggregator group; the client has since obtained the `ross.` subdomain from that group, so the constraint behind the clause is gone. **Anything that hardcodes or assumes `sites.leadconnectorhq.com` or `info.enchargecapital.com` for the funnel is wrong.** Checked 12 Sep: nothing in `src/`, `supabase/`, `web/` or the tests carries either address — the only `leadconnectorhq` hits are the GHL API base `services.leadconnectorhq.com` (`.env.example`, GHL-AUDIT), a different host and unchanged. `src/lib/voice/rules.ts` and `VOICE.md` cite D30 for the *path*, which is unchanged — no edit. **Not verified here — open:** whether the Refi Pixel install and the Conversions API `Lead` event (D31) survived the domain move | 7 Sep (recorded 12 Sep) |
| **D81** | **D9 is NARROWED, not replaced. Two-turn confirmation applies to tool calls the ASSISTANT makes on its own initiative — and still stands for those. It does NOT apply to a direct action the user takes in the interface: when the client drags a lead from one stage to the next, that click IS the confirmation.** No dialog on a stage change | D9 (08 Aug) was written against an AI writing to a live CRM unconfirmed. A person moving a card is not that: the intent is in the gesture, and a dialog on every stage change would make the board unusable. What a user-initiated write still owes, from rules that already exist: idempotent and keyed on the GHL opportunity ID (rule 9); never blindly retried after a timeout (rule 8); recorded in `audit_log` with the person as actor; a refused or failed write shown in words with the board put back, never swallowed (rule 7). D71's reading of D9 — a standing rule that binds the day an assistant-initiated write tool exists — is unchanged | 12 Sep |
| **D82** | **Explicitly out of scope, and stated to the client in writing:** a drag-and-drop dashboard *builder* · agent software that carries out work on its own · the Quickli lending calculator (no open API — partner access only) · the client's separate finance CRM (same reason) | Quickli and the finance CRM are closed to us by their vendors, not by this project. The first two draw the line around Milestones 4 and 6: a fixed screen the client uses rather than composes, and an assistant that answers and drafts rather than acts unattended (D9/D81 govern the writes it may make). Check every "can it also…" against this list before it becomes a task (R8). The 08 Aug "separate finance CRM — price separately when raised" line in §4 is now settled by this | recorded 12 Sep |
...
(5) `en-AU` short September is "Sept". (7, after go-live) **A second, page-level scrollbar on the client's 1920×945 laptop at 150 % zoom (a 1280×630 viewport):** `.sr-only` is `position: absolute` and `.shell` was not positioned, so the hidden spans of stage rows scrolled out of view inside the overview positioned against the page and made the document 722 px tall in a 630 px window. `.shell { position: relative }`; `overview.spec.ts` now asserts no document scroll at 630 px. (6) **A staged deletion (`git rm NotYet.tsx`, part-2
...
**Not verified:** the stack half (`overview.test.ts`) ran only in CI (run 34739663881 covered
Part 1; the reviewer pushed this part as `4a0b5ac`, CI #55 green); **live (13 Sep, after the
reviewer's push):** migration applied via MCP (CLI token 401 — see "Known broken" for the
version-row repair), `crm` v1 deployed via MCP, secrets NOT set (needs `supabase login`), so no
refresh has yet run from the app against the live project; five-digit counts are proven at unit level (`formatCount`) and by the tile's
...
`20260912010000_ghl_sync.sql`: six tables (`ghl_pipelines`, `ghl_stages`, `ghl_opportunities`,
`ghl_contacts`, `ghl_custom_fields`, `ghl_sync_runs`), RLS enabled **and forced** with the
staff-allowlist SELECT policy, explicit anon/authenticated/service_role grants, and three
`service_role`-only functions — `begin_ghl_sync_run` (one running slot per pipeline via a
partial unique index; a run left `running` past `GHL_STALE_RUN_AFTER_SECONDS` is retired as
`failed/STALE`; a second starter gets SQLSTATE 55006, never a race), `apply_ghl_snapshot`
(the whole write phase in ONE transaction, unchanged rows untouched by `content_hash`,
removals only from a complete read), `finish_ghl_sync_run`. **Code** `src/lib/crm/ghl/`:
`config.ts` (the one reader of `GHL_PRIVATE_INTEGRATION_TOKEN`), `types.ts` (Zod, strips the
PII-bearing `relations`/`attributions`/attribution blocks, normalises dates to UTC),
`map.ts` (snapshot rows, SHA-256 content hash, `readCustomField` by type), `client.ts`
(GET only; timeout/backoff/breaker via http.ts; paces on `x-ratelimit-remaining`; cursor
pagination with a page cap that fails rather than truncates; 401 → a LOUD
`UNAUTHENTICATED` "this is NOT an empty pipeline"), `sync.ts` (the run), `store.ts`
(supabase-js over the three RPCs). **Trigger** `npm run crm -- sync` (and `-- read`, a
no-database live read that prints ids, counts and a snapshot hash; `-- runs`). **Seed:**
eleven `ghl_field_map` contact rows from the 12 Sep read. `.env.example` documents
`GHL_PIPELINE_ID=M4unnMKBy0TgwCwOA6wS`, `GHL_CUSTOM_FIELD_FOLDER_IDS`, the caps. **Tests:**
90 unit (`tests/unit/crm/ghl/`: config 15 · map 24 · client 23 · sync 20 · store 8),
`tests/integration/ghl-sync.test.ts` (7, real stack: first run row by row; **idempotency —
the same sync again reports everything unchanged and the five mirror tables are
byte-identical**; the shape changing underneath; marks cleared on return; overlap refused +
stale retired without a race; partial run keeps the failed contact's row; dead token applies
nothing), `tests/security/ghl.test.ts` (5 static + 5 stack), `schema.test.ts` extended
(EXPECTED_TABLES, the eleven contact rows pinned, `scope` pinned). Docs: `SCHEMA.md` §3 + §8,
`SECURITY.md` §12, `TASKS.md`, `CLAUDE.md` §5.
**Decided (Part A, before code):** (1) *Store* opportunities + stage + pipeline, the contact
behind each, and the custom-field definitions in play; NOT `relations`/`attributions` (IP,
user agent, pixel id), NOT `consumer_leads` (its NOT NULL `lead_type`/`lead_source`/
`pipeline_stage` enums would force invented values and cannot hold a stage GHL added — a
later part may project mirror → leads once R16's translation is decided). (2) *Truth* = GHL;
local rows keyed on the GHL id with a content hash; a disagreement is always resolved by
overwriting local. **An unknown pipeline id returns 200 and an empty list** (observed), so
the sync verifies `GHL_PIPELINE_ID` against the pipelines list first and refuses otherwise.
(3) *Full sync every run*: incremental by `updatedAt` cannot see deletions or another
pipeline's move, and the pipeline is 10 opportunities today; caps (`GHL_MAX_PAGES` 50 ×
100, `GHL_MAX_CONTACTS_PER_RUN` 2,000) make "too big" a loud failure. Revisit at ~2,000
contacts/run (2,000 GETs ≈ 200 s at 100 requests per 10 s) — then an incremental contact read
(POST contacts/search, a write verb by shape) becomes necessary. (4) *Failure posture*: the
write phase is one transaction, so an interrupted sync leaves the mirror untouched or fully
applied; removals need a complete, fully-valid read; every run row carries every count and
every failed id; a 401/403/open-breaker aborts, a single bad contact makes the run `partial`.
(5) *Custom fields*: values stored as `{fieldId: raw}` jsonb, definitions mirrored by id with
name/type/folder and `removed_at`; a rename changes a definition, a removal marks it, neither
touches a stored value; the eleven internal names live in `ghl_field_map`. *Ownership*: no
`user_id` (no author to name — inventing one is the failure the memory-layer comment warns
about); `scope` carried and pinned to `'workspace'` like `memory_chunks` so the policy shape
matches and a private variant is additive. *Deleted in GHL* = `removed_at` mark, never a
delete (the row stays explainable; no FK opportunity → stage or → contact for the same reason).
**Live read (authorised, GET only, 12 Sep):** pipelines 200 — **six**, not the audit's five
(the Finance Pipeline is the sixth), ten stages, ids identical to the seed; opportunities
search scoped to `M4unnMKBy0TgwCwOA6wS`: **10, all `open`, all `monetaryValue` 0, all with a
contact**, 7 Appointment Booked · 2 New Lead · 1 Contacted; cursor `startAfterId`/`startAfter`
verified across pages, `nextPage` is `2` mid-way and `""` on the last page; contact by id
200 with `customFields: [{id, value}]` (string for a dropdown, string[] for a multi-select);
custom fields 200 — **57 definitions**; rate-limit headers 100 per 10,000 ms, 200,000/day.
**Two consecutive `crm -- read` runs: identical snapshot hash
`de168def…c8f4f6`, 13 requests each, 0 rejections.** The migration + functions + seed were
then run on the live project inside one `BEGIN…ROLLBACK` via the Supabase MCP, results
returned in a deliberate exception: 6 tables RLS-forced · 6 SELECT policies · anon 0 grants
· service_role 24 DML grants · functions service_role-only · second begin → 55006 · apply
twice → all `unchanged`, rows byte-identical · changed snapshot → 1 removed, 1 renamed by id,
contact kept · stale → `failed/STALE` · scope `'user'` → 23514. Verified afterwards: 0 ghl
tables, 0 functions, 0 contact map rows on live. Nothing was applied.
**Surprised by:** (1) **The Stage 1 folder `BEFyPDjs8dlcpRuz3ZcL` holds NINE fields, not
ten.** "Current Interest Rate" is not in it: `7pR62a3yZcOtnmF2sJrQ` "Current Interest Rate
(%)" (TEXT, 9 Aug, folder `mDYlnuPHIIMlif6jwX6X`) and `wFamEJlm7JJcNK7W6nfW` `interest_rate`
(NUMERICAL, 30 Jun) both predate Stage 1 and were made by someone else. The sign-off record
says ten. (2) **Someone added a folder `fA9zYqgDoZUUN5CKnb5G` on 19–21 Aug** — the fields the
live form now writes: "Roughly how much is left on your home loan?" (`TANd0sfC9wRwuJKhSGFx`)
and "What's your current interest rate?" (`hX8JQblBT9iJhYEa348M`), both MULTIPLE_OPTIONS
bands — mapped as `loan_balance` and `current_interest_rate`; plus an earlier duplicate
"…left on your loan?" (`JYVIKXP7EvyihZ2vtB3o`, missing the $750k–$1m band), a
"Who's Your Current Lender?", and **two "Are you employed or self-employed?" fields, one
whose options are STATES (QLD, VIC, NSW…) and one whose options are "Option 1/2/3"** — a
mislabelled field the live contacts carry. (3) The audit's `contactName` is absent on
`GET /contacts/{id}`; `firstName`/`lastName` are what comes back. (4) No opportunity carries a
monetary value — every one is 0, so the board's "value" column will be zeros until someone
enters them in GHL. (5) The brief calls itself "Milestone 4, Part 1 — GoHighLevel read and
sync layer" while `TASKS.md` M4 part 1 is "The screen" — numbering left as the reviewer's call;
recorded as a sub-line under M4.
**Not verified:** CI has not run (do not commit / push); the stack suites ran nowhere but
inside the rolled-back live transaction (the SQL half) — `tests/integration/ghl-sync.test.ts`
and the stack half of `tests/security/ghl.test.ts` have not executed against a real stack;
`SUPABASE_DB_URL` is unset locally so they skipped here (5 files, 32 tests skipped
locally). No sync has been run into any database. Whether the token can still reach every
route it needs at handover scope is unchanged (R21).
**Next:** reviewer reads this → push → CI (the first run of the two new stack suites) → apply
the migration through the CLI → one `npm run crm -- sync` against the live project and
`-- runs` to read the row → M4 part 2 wraps `runGhlSync` in a server endpoint the board calls.

---

### 2026-09-12 — [docs] Eight milestones replace the six stages; Milestone 4 defined; funnel domain moved; D9 narrowed

**Did:** documentation only — no code, migrations, tests or CI touched; nothing committed.
Recorded the 1 Sep agreement (**D77**: eight milestones, 2,000 USD total; 1–3 complete, signed
off and paid — 600 received, 1,400 remaining), Milestone 4 "Your data on screen" (**D78**), the
5 Sep ordering change (**D79**), the 7 Sep move of the funnel to `ross.fundd.com.au` and of
email sending to `send.fundd.com.au` (**D80**), the narrowing of D9 for user-initiated writes
(**D81**) and the explicit out-of-scope list (**D82**). D26, D27 and the domain clause of D30
are struck through in place, each pointing at its replacement; nothing deleted.
`PHASE-ACCEPTANCE.md` now opens with the eight-milestone map, carries Milestone 4's criteria in
the Stage 2 style and starting points for 5–8; the old Stage 4–6 sections are struck through in
place. `TASKS.md` gains a Milestone 4 section (five parts, no task detail — that comes with each
part's brief) and marks the Stage 3–6 outlines superseded. `CLIENT-CONTEXT.md` §12 rows 10 and
12 superseded by strikethrough, row 14 added for the sending domain, §13 F updated; **§1
untouched, no section renumbered.** `CLAUDE.md`, `README.md`, `SECURITY.md`, `SCHEMA.md` §2a
and `RUNBOOK.md` §10 re-pointed with one-line touches.

**Decided:** D77–D82 above.

**Found, not settled — left as written on both sides:** (1) Stage 1 is recorded as **198**
received on 22 Aug; the milestones document prices Milestone 1 at **200** and records **600**
across 1–3. (2) `TASKS.md`: the two sign-off tasks (2.7.4, 3.12) are ticked on the strength of the
milestones document; 2.7.1–2.7.3 and the unbuilt Stage 3 items (3.3–3.7, 3.10–3.11) are marked
superseded, not ticked, because no test run or acceptance evidence was produced in this session. (3) D71 places n8n at "Stage 6" and the
review-queue surface at "Stage 4"; where those land among milestones 5–8 is decided at the
relevant kickoff. (4) Whether the Refi Pixel install survived the funnel's domain move is
unverified. (5) The pre-5 Sep milestone order is not on file.

**Next:** the Milestone 4 part 1 brief.
...
| 01 Sep | — | **Eight milestones at 2,000 USD total** agreed and confirmed in a milestones document the client holds: 1 CRM and ad tracking set up (200) · 2 Foundations and the client's writing voice (200) · 3 Memory and the app itself (200) · 4 Your data on screen (350) · 5 Files, documents and research (250) · 6 Talking to it, and your day (350) · 7 Advertising and social (300) · 8 Content and handover (150). **Milestones 1–3 complete, signed off and paid — 600 received** | **Binding.** D77, D78 — supersedes D26, D27 |
| 05 Sep | — | Client asked for the dashboard first, then the files and voice milestones, with advertising and social moved later | Agreed — D79; the 01 Sep list above is the current order |
| 07 Sep | — | FUNDD funnel live on **`ross.fundd.com.au`** (subdomain obtained from the aggregator group); email sends from **`send.fundd.com.au`**; `info.enchargecapital.com` now serves a different funnel | Recorded — D80, supersedes the domain clause of D30 |
| Sep (date not recorded) | in writing | Stated to the client as out of scope: a drag-and-drop dashboard builder, agent software that acts on its own, the Quickli lending calculator (no open API), the separate finance CRM (same reason) | Recorded — D82 |
